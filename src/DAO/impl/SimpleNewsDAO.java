...
public class SimpleNewsDAO implements NewsDAO {
    ...
    public ArrayList<News> getNewsforList() {
        try {
            PreparedStatement st = conn.prepareStatement(
                    "select n.id, u.login, n.header, n.date from news as n inner join users u on n.author_id = u.id where n.accepted = true  limit 5");
    ...
    public News getNewsByID(int i) {
        try {
            PreparedStatement st = conn.prepareStatement("select u.login,u.description,u.name,n.header,n.text,n.date,t.nam,t.link from news as n inner join users as u on n.author_id = u.id inner join topic as t on n.topic_id = t.id where n.id=?");
            st.setInt(1,i);
            ResultSet rs = st.executeQuery();
            rs.next();
            return new News(i,rs.getString("login"),rs.getString("name"),rs.getString("description"),new Topic(rs.getString("nam"),rs.getString("link")),rs.getString("header"),rs.getString("text"),rs.getString("date"));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return null;
